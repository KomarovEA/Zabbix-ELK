...
mysql -uroot -e "create database $3 character set utf8 collate utf8_bin";
mysql -uroot -e "grant all privileges on $3.* to $3@localhost identified by '$4'";
#mysql --user=zabbix --password=zabbix < database/mysql/schema.sql
#mysql --user=zabbix --password=zabbix < database/mysql/images.sql
#mysql --user=zabbix --password=zabbix < database/mysql/data.sql
...
#zcat /usr/share/doc/zabbix-server-mysql-4.4.1/create.sql.gz | mysql --database=$2 --user=$3 --password=$4
...
#yum -y install http://repo.zabbix.com/zabbix/4.2/rhel/7/x86_64/zabbix-web-4.2.7-1.el7.noarch.rpm
